...
MONGO_URI = os.getenv("MONGO_URI", mongosh "mongodb://mongo:CZRyLpoTQNPPeQpcoMPyjYwNIoBmCdbi@shinkansen.proxy.rlwy.net:14794")
...
async def summarize_email(email_body):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(OLLAMA_API_URL, json=payload) as response:
                if response.status != 200:
                    raise Exception(f"AI API returned {response.status}")
                result = await response.json()
