...
CLIENT_SECRET = os.getenv("GOOGLE_CLIENT_SECRET")
TOKEN_URI = "https://oauth2.googleapis.com/token"
OLLAMA_BASE_URL = os.getenv("OLLAMA_API_URL", "https://ollama-on-render.onrender.com")  # Base URL without /api
HUGGINGFACE_API_URL = "https://api-inference.huggingface.co/models/distilgpt2"
HUGGINGFACE_API_TOKEN = os.getenv("HUGGINGFACE_API_TOKEN")  # Add your HF token in .env
MONGO_URI_RAW = os.getenv("MONGO_URI", "mongodb+srv://vinitsonawane76:VPeMCZGJOKEmtgbM@cluster0.on6kpbz.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
MONGO_URI = MONGO_URI_RAW.replace('mongosh', '').replace('"', '').strip()
DB_NAME = "ai_email_agent"
EMAIL_CACHE_TTL = int(os.getenv("EMAIL_CACHE_TTL", 60))
...
async def generate_email(prompt):
    ...
    async with aiohttp.ClientSession() as session:
        # Attempt 1: Ollama with gemma:2b
        try:
            tags_url = f"{OLLAMA_BASE_URL}/api/tags"
            logger.debug(f"Fetching models from {tags_url}")
            async with session.get(tags_url, timeout=aiohttp.ClientTimeout(total=10)) as model_response:
                ...
                available_models = [model["name"] for model in models_data.get("models", [])]
                logger.debug(f"Available models: {available_models}")
                matching_model = next((m for m in available_models if m.startswith("gemma")), "gemma:2b")
                logger.debug(f"Selected model: {matching_model}")

            payload = {"model": matching_model, "prompt": email_prompt, "stream": False}
            generate_url = f"{OLLAMA_BASE_URL}/api/generate"
            logger.debug(f"Sending request to Ollama at {generate_url} with payload: {payload}")
            async with session.post(generate_url, json=payload, timeout=aiohttp.ClientTimeout(total=20)) as response:
                response_text = await response.text()
                logger.debug(f"API response: {response.status} - {response_text}")
                if response.status != 200:
                    raise Exception(f"Ollama API returned {response.status}: {response_text}")
                result = await response.json()
                generated_email = result.get("response")
                if not generated_email or "Could not generate" in generated_email:
                    raise Exception("Ollama returned empty or invalid response")
                ai_cache[cache_key] = generated_email
                logger.info("Email generated by Ollama (gemma:2b)")
                return generated_email
        except Exception as e:
            logger.error(f"Ollama (gemma:2b) failed: {str(e)}")

        # Attempt 2: Hugging Face with distilgpt2
        try:
            if not HUGGINGFACE_API_TOKEN:
                raise Exception("Hugging Face API token not provided")
            logger.debug(f"Attempting Hugging Face at {HUGGINGFACE_API_URL}")
            headers = {"Authorization": f"Bearer {HUGGINGFACE_API_TOKEN}"}
            payload = {"inputs": email_prompt, "parameters": {"max_length": 200, "do_sample": True}}
            async with session.post(HUGGINGFACE_API_URL, json=payload, headers=headers, timeout=aiohttp.ClientTimeout(total=20)) as response:
                if response.status != 200:
                    raise Exception(f"Hugging Face API returned {response.status}: {await response.text()}")
                result = await response.json()
                generated_email = result[0].get("generated_text") if result else ""
                if not generated_email or len(generated_email.strip()) < 50:
                    raise Exception("Hugging Face returned insufficient response")
                ai_cache[cache_key] = generated_email
                logger.info("Email generated by Hugging Face (distilgpt2)")
                return generated_email
        except Exception as e:
            logger.error(f"Hugging Face (distilgpt2) failed: {str(e)}")
            raise Exception("All AI providers failed to generate email")
...
async def summarize_email(email_body):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            payload = {"model": matching_model, "prompt": f"Summarize this email in 2-3 sentences: {email_body}", "stream": False}
            generate_url = f"{OLLAMA_BASE_URL}/api/generate"
            async with session.post(generate_url, json=payload, timeout=aiohttp.ClientTimeout(total=20)) as response:
                if response.status != 200:
                    raise Exception(f"AI API returned {response.status}: {await response.text()}")
