...
app = Flask(__name__)
# Simplified CORS configuration with broader origin support for debugging
...
CLIENT_SECRET = os.getenv("GOOGLE_CLIENT_SECRET")
TOKEN_URI = "https://oauth2.googleapis.com/token"
HF_API_TOKEN = os.getenv("HF_API_TOKEN")  # Optional for Hugging Face API
MONGO_URI = os.getenv("MONGO_URI", "mongodb+srv://vinitsonawane76:VPeMCZGJOKEmtgbM@cluster0.on6kpbz.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0").replace('mongosh', '').replace('"', '').strip()
DB_NAME = "ai_email_agent"
EMAIL_CACHE_TTL = int(os.getenv("EMAIL_CACHE_TTL", 60))
AI_CACHE_TTL = int(os.getenv("AI_CACHE_TTL", 300))
...
# Load Hugging Face model (using t5-small for lower resource usage)
try:
    logger.info("Loading Hugging Face model (t5-small)...")
...
async def generate_email(prompt):
    ...
    try:
        input_prompt = (
            f"Generate a professional email based on this request: '{prompt}'. "
            f"Include a subject line starting with 'Subject:', a greeting (e.g., 'Dear [Recipient]'), "
            f"a polite body, and a closing (e.g., 'Best regards, [Your Name]'). "
            f"Use plain text with line breaks."
        )
        logger.debug(f"Generating email with prompt: {input_prompt}")
        generated = generator(input_prompt, max_length=300, min_length=100, num_beams=4)
        generated_email = generated[0]['generated_text']
...
async def summarize_email(email_body):
    ...
    if cache_key in ai_cache:
        return ai_cache[cache_key]
    try:
        summary = generator(f"summarize: {email_body}", max_length=100, min_length=30, num_beams=4)
...
async def categorize_email(email_body):
    ...
    if cache_key in ai_cache:
        return ai_cache[cache_key]
    try:
        input_prompt = f"Categorize this email as Work, Personal, Newsletter, Spam, or Other: {email_body}"
        result = generator(input_prompt, max_length=20, min_length=5, num_beams=4)
...
async def send_email(access_token, refresh_token, recipient, subject, message):
    try:
        ...
        sent_msg = service.users().messages().send(userId='me', body={'raw': base64_message}).execute()
        save_sent_email_to_db(recipient, subject, message)
        return sent_msg['id'], new_access_token
    except Exception as e:
        logger.error(f"Failed to send email: {str(e)}")
...
def store_tokens():
    ...
    try:
        ...
        return jsonify({"status": "success", "message": "Tokens stored"})
    except Exception as e:
...
async def send_email_endpoint():
    ...
    if not auth_header or not auth_header.startswith('Bearer '):
        return jsonify({"error": "Invalid Authorization header"}), 401
    access_token = auth_header.split('Bearer ')[1].strip()
    if not refresh_token or not data or not all(k in data for k in ['recipient', 'subject', 'message']):
        return jsonify({"error": "Missing required fields"}), 400
    try:
        email_id, new_access_token = await send_email(access_token, refresh_token, data['recipient'], data['subject'], data['message'])
