...
async def generate_email(prompt):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            payload = {
                "model": "gemma:2b",
                "prompt": f"Write a professional email for: '{prompt}'. Format: Subject: [Subject]\nDear [Recipient],\n[Body]\nBest regards,\n[Your Name]. Use plain text with line breaks.",
                "stream": False,
                "temperature": 0.7,
                "max_tokens": 150
            }
            ...
            async with session.post(generate_url, json=payload, timeout=aiohttp.ClientTimeout(total=30)) as response:
                response_text = await response.text()
                if response.status != 200:
                    logger.warning(f"Ollama failed with status {response.status}: {response_text}")
                    return await generate_email_with_hf(prompt, session)
                result = await response.json()
                logger.debug(f"Ollama response: {result}")
                generated_email = result.get("response", "").strip()
                # Validate email structure
                if not generated_email or not all(keyword in generated_email for keyword in ["Subject:", "Dear", "Best regards"]):
                    logger.warning(f"Ollama returned invalid email: {generated_email}")
                    return await generate_email_with_hf(prompt, session)
                ai_cache[cache_key] = generated_email
                logger.info("Email generated successfully with Ollama")
                return generated_email
...
async def generate_email_with_hf(prompt, session):
    ...
    try:
        payload = {
            "inputs": f"Write a professional email for: '{prompt}'. Format: Subject: [Subject]\nDear [Recipient],\n[Body]\nBest regards,\n[Your Name]. Use plain text with line breaks.",
            "parameters": {
                "max_length": 150,
                "temperature": 0.7,
                "top_p": 0.9,
