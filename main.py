...
#         "http://localhost"
#     ]
#     if origin and any(origin.startswith(prefix) for prefix in allowed_origins):
#         response.headers['Access-Control-Allow-Origin'] = origin
#         logger.debug(f"Set CORS origin to {origin}")
#     else:
#         response.headers['Access-Control-Allow-Origin'] = 'https://email-extension.onrender.com'
#         logger.debug(f"Set default CORS origin for {origin}")
...
#     try:
#         if emails_collection and not emails_collection.find_one({'id': email_data['id']}):
...
#             }
#             emails_collection.insert_one(email_data)
#             logger.debug(f"Saved sent email to {recipient} to DB")
#     except Exception as e:
...
#                 "max_tokens": 300
#             }
#             logger.debug(f"Attempt {attempt + 1}: Sending request to Ollama at {OLLAMA_API_URL}/api/generate")
#             async with session.post(f"{OLLAMA_API_URL}/api/generate", json=payload, timeout=aiohttp.ClientTimeout(total=60)) as response:
#                 if response.status != 200:
#                     logger.warning(f"Ollama attempt {attempt + 1} failed with status {response.status}: {await response.text()}")
#                     if attempt == retries - 1:
#                         raise Exception(f"Ollama failed with status {response.status}")
#                     await asyncio.sleep(backoff * (2 ** attempt))
#                     continue
#                 result = await response.json()
...
#             except Exception as e:
#                 logger.error(f"Ollama failed, using fallback: {str(e)}")
#                 # Fallback
...
#                 cache_key = f"generate_fallback_{hash(prompt)}"
#                 ai_cache[cache_key] = generated_email
...
#                 "max_tokens": 100
#             }
#             logger.debug(f"Sending summarize request to Ollama at {OLLAMA_API_URL}/api/generate")
#             async with session.post(f"{OLLAMA_API_URL}/api/generate", json=payload, timeout=aiohttp.ClientTimeout(total=60)) as response:
#                 if response.status != 200:
#                     raise Exception(f"AI API returned {response.status}: {await response.text()}")
#                 result = await response.json()
#                 summary = result.get("response", "Could not summarize email").strip()
...
#     if cache_key in ai_cache:
#         logger.debug("Returning cached category")
#         return ai_cache[cache_key]
#     try:
#         async with aiohttp.ClientSession() as session:
#             async with session.get(f"{OLLAMA_API_URL}/api/tags", timeout=aiohttp.ClientTimeout(total=5)) as model_response:
...
#                 "max_tokens": 20
#             }
#             logger.debug(f"Sending categorize request to Ollama at {OLLAMA_API_URL}/api/generate")
#             async with session.post(f"{OLLAMA_API_URL}/api/generate", json=payload, timeout=aiohttp.ClientTimeout(total=60)) as response:
#                 if response.status != 200:
#                     raise Exception(f"AI API returned {response.status}: {await response.text()}")
#                 result = await response.json()
...
#     try:
#         logger.info("Processing fetch-emails request")
...
#         return jsonify({"error": "Missing required fields: recipient, subject, message"}), 400
#     try:
#         logger.info(f"Processing send-email request to {data['recipient']}")
...
def save_email_to_db(email_data):
    try:
        if emails_collection is not None and not emails_collection.find_one({'id': email_data['id']}):
            email_data['created_at'] = datetime.now(timezone.utc)
            email_data['updated_at'] = datetime.now(timezone.utc)
            emails_collection.insert_one(email_data)
            logger.debug(f"Saved email {email_data['id']} to DB")
